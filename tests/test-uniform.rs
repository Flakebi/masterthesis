...
fn main() {
    ...
    let (mut swapchain, images) = {
        ...
        println!("Choosing image format {:?}", format);
...
void main() {
    frag_position = vec2(0.0, position.y);
...
void main() {
    ...
    if (frag_position.x <= 0.1) {
        f_color = vec4(1, 0, 0, 1);
    } else {
        vec4 i = vec4(frag_position.x, frag_position.y, frag_position.x, frag_position.y);
        i.x += 1;
        i = i * 2 + vec4(1);
        float j = frag_position.y;
        i /= j;
        i += j;
        f_color = i;
    }
...
"
    ...
    loop {
        frames += 1;
        let passed = starttime.elapsed();
        if passed > Duration::from_millis(250) && frames > 10 {
            let fps = frames as f32 / passed.as_secs_f32();
            frames = 0;
            println!("{} fps", fps);
    ...
    let passed = perf_starttime.elapsed();
