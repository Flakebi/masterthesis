...
After instrumenting the shaders and running a benchmark, each basic block is associated with a counter which tells us how often each block was executed.
...
In previous tests with PGO, Ashes did not show these artifacts, so maybe this bug is connected with a recent update that Ashes received.

\subsection{Unused code}
\label{sub:eval_unused}
Looking at basic blocks which get never executed, the compiler can remove them and study the changes in register usage and performance.
...
Using PGO, the compiler detects that all instructions in all but one case statements are never executed.
Thus, in the optimized shader version all these basic blocks are gone.
...
The register usage for our switch vm is not shown because it stays unchanged, only one vector register less is used and the possible occupancy does not change.
...
Surprisingly, Dota 2 does not show a great effect when removing code, even when nearly \SI{20}{\percent} of the basic blocks are removed.
...
As before, removing blocks is only done for Dota 2, Mad Max and the switch vm, the other games do not work with the current implementation.
